...
def analyze_text(text):
    ...
    stemmer = PorterStemmer() # reduces to root 
    lemmatizer = WordNetLemmatizer() # reduces to base 
    ...
    # Count frequencies
    token_freq = FreqDist(filtered_tokens) # frequency in tokens 
    stem_freq = FreqDist(filtered_stems) # frequency in stems 
    lemma_freq = FreqDist(filtered_lemmas) #freqeuncy in lemmas 
    
    print("=== TOKENIZATION RESULTS ===")
    print(f"Total tokens: {len(tokens)}")  #total tokens 
    print(f"Unique tokens: {len(set(tokens))}") #unique tokens 
    print("\n20 Most common tokens:")
    for token, count in token_freq.most_common(20):  #counts most common 
        print(f"{token}: {count}")
    
    print("\n=== STEMMING RESULTS ===")
    print("\n20 Most common stems:")
    for stem, count in stem_freq.most_common(20): #counts most common 
        print(f"{stem}: {count}")
    
    print("\n=== LEMMATIZATION RESULTS ===")
    print("\n20 Most common lemmas:")
    for lemma, count in lemma_freq.most_common(20): # counts most common 
        print(f"{lemma}: {count}")
    ...
    for trigram, count in trigram_freq.most_common(20): # count most common 
    ...
    for ent in doc.ents:
        if ent.label_ not in entities:
            entities[ent.label_] = []  #create list 
        if ent.text not in entities[ent.label_]:
            entities[ent.label_].append(ent.text) #add unique text
    ...
    for label in entities:
        print(f"\n{label}:")
        for entity in entities[label]: # type like person 
